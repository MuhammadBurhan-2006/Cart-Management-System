...
public:
    ...
    Customer(int id, string n, string user, string pass, string ph) {
        customerID = id;
        name = n;
        username = user;
        password = pass;
...
class Bill {
private:
    Cart* cart; //pointer to the customer cart
...
public:
    ...
    void applyCoupon(string code) {
        ...
        double currentSubtotal = cart->getSubtotal();
        if (code == COUPON_1_CODE) {// "SAVE10"
            couponCode = code;
            couponDiscount = currentSubtotal * COUPON_1_DISC;
            couponApplied = true;
            cout << "Coupon applied: 10% off!" << endl;
        }
        else if (code == COUPON_2_CODE) {// "FLAT50"
            couponCode = code;
            couponDiscount = COUPON_2_DISC;
            couponApplied = true;
            cout << "Coupon applied: Flat Rs.50 off!" << endl;
        }
        else if (code == COUPON_3_CODE) {// "FRESH20"
            couponCode = code;
            couponDiscount = currentSubtotal * COUPON_3_DISC;
            couponApplied = true;
